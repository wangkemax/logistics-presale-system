...
| | CI/CD | 🔲 待做 | GitHub Actions + Docker 构建 |
...
| 模块 | 任务 | 状态 | 说明 |
|------|------|------|------|
| **文档生成** | 标书 Word 生成器 | 🔲 待做 | python-docx, 公司模板 + AI 内容填充 |
| | 方案 PPT 生成器 | 🔲 待做 | python-pptx, 10-15 页标准结构 |
| | PDF 导出 | 🔲 待做 | LibreOffice 转换 |
| **前端增强** | 方案设计工作台 | 🔲 待做 | Agent 输出可视化 + 人工调整 + 方案对比 |
| | 报价工作台 | 🔲 待做 | 参数联动 + 实时计算 + 多方案对比 |
| | 标书编辑器 | 🔲 待做 | 富文本 + AI 章节重写 + 版本对比 |
| | QA 审核面板 | 🔲 待做 | 问题详情 + 修复追踪 + 一键重跑 |
| | 知识库浏览页 | 🔲 待做 | 搜索 + 详情 + 收藏 |
| **Agent 优化** | Prompt 精调 | 🔲 待做 | 基于真实招标文件迭代 prompt |
| | 多方案并行 | 🔲 待做 | 方案 A/B/C 并行设计 + 对比 |
| | Agent 缓存 | 🔲 待做 | Redis 缓存相同输入的 Agent 输出 |
...
| 客户协作门户 | 客户端查看方案 + 在线反馈 |
| 项目模板 | 按行业预设模板 (电商/冷链/制造) |
| 报价审批流 | 顾问提交 → 经理审核 → 客户发送 |
| 数据分析看板 | 投标成功率 / 报价趋势 / Agent 用量 |
| 移动端适配 | 响应式 + PWA |
